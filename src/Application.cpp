...
void CAE::Application::editorDragDropLogic()
{
    ...
    {
        return "rect #" + std::to_string(p.getId()) + ": " + "w:" + std::to_string(p.getRect().width) + ", h:" +
               std::to_string(p.getRect().height);
    };
...
void CAE::Application::loadState()
{
    if(ifstream open("config.json"); open.is_open())
    {
        ...
        try
        {
            ...
            auto p = appSettings.at("currentAsset").get<std::string>();
...
void CAE::Application::loadAsset(const std::string& path, bool setAsCurr)
{
    if(!path.empty())
    {
        ...
        //task(path);
        std::async(std::launch::async, task, path);
