...
{
	int pos = -47 + 8;
	for (iterator tool = tools.begin(); tool != tools.end(); ++tool)
	{
		auto& [f, s] = *tool;
...
{
	tools[tool_type::MOVEPART] = std::make_shared<MovePen>(app->eManager, app->ico_holder.getTexture(ico_t::Move), *w, app->useFloat);
	tools[tool_type::MAGICTOOL] = std::make_shared<_MagicTool>(app->eManager, app->ico_holder.getTexture(ico_t::Magic), *w, app->useFloat);
	tools[tool_type::MOVEVIEW] = std::make_shared<MoveView>(app->eManager, app->ico_holder.getTexture(ico_t::Hand), *w, app->view, app->useMouse);
	current_tool = tools.find(tool_type::MOVEPART);
}

void CAE::Tools::update()
{
	current_tool->second->update(data);
	/*for (auto& [_, t] : tools)
		t->update();*/
}
